...
if (!window.__TOTEM_LEV_INITIALIZED__) {
  ...
  window.enviarARevision = function() {
    ...
    // Snapshot del levantamiento congelado para la revisión
    sessionStorage.setItem('totem_lev_para_revision', JSON.stringify({
      lev: window.lev,
      enviado_en: new Date().toISOString(),
      estado: 'EN_REVISION',
    }));
    // Limpiar el levantamiento actual (queda libre para empezar uno nuevo después)
    sessionStorage.removeItem(STORAGE_KEY);
    sessionStorage.removeItem('totem_oferta_actual');
    sessionStorage.removeItem('totem_cotizacion_actual');
    window.location.href = 'revision.html';
  };

  // Alias para retrocompatibilidad
  window.enviarACotizar = window.enviarARevision;

  window.confirmarLimpiar = function() {
    if (!confirm('¿Borrar el levantamiento actual y empezar uno nuevo? Esta acción no se puede deshacer.')) return;
    sessionStorage.removeItem(STORAGE_KEY);
    window.lev = { proyecto: { cliente:'', contacto:'', telefono:'', referencia:'', lugar:'', coords:'', problema:'' }, items: [], requerimientos: [] };
    window.nextLevId = 1; window.nextReqId = 1;
    window.aplicarProyectoAUI();
    window.renderLista();
    window.renderRequerimientos();
    alert('Levantamiento limpiado. Puedes empezar uno nuevo.');
  };
...
var enviarARevision = window.enviarARevision;
var confirmarLimpiar = window.confirmarLimpiar;
