...
function cargarDesdeLevantamiento() {
  ...
  if (!lev || !lev.lev || !lev.lev.items || lev.lev.items.length === 0) {
    // Sin levantamiento: mostrar mensaje y redirigir
  ...
  oferta.items = lev.lev.items.map(litem => {
    ...
    return {
      ...litem,
      // Overrides comerciales (no estaban en el levantamiento):
      margen_pct: prev?.margen_pct ?? null,           // null = usar global
      horas_mo_override: prev?.horas_mo_override ?? null, // null = usar receta
    };
  });

  // Restaurar adicionales y opciones de oferta previa
  if (ofertaPrevia) {
    oferta.adicionales = ofertaPrevia.adicionales || [];
    if (ofertaPrevia.opciones) Object.assign(oferta.opciones, ofertaPrevia.opciones);
    if (ofertaPrevia.nextAdicId) nextAdicId = ofertaPrevia.nextAdicId;
  }
...
function renderAdicionales() {
  ...
  if (oferta.adicionales.length === 0) {
    cont.innerHTML = `<div class="empty-state" style="padding:24px;"><p>No hay costos adicionales agregados.</p></div>`;
    return;
  }
      ...
      <div class="cart-line" data-id="${a.id_temp}" style="grid-template-columns: 1fr auto auto;">
        <div>
          ...
          <div class="line-params" style="grid-template-columns: 1fr 1fr 1fr; margin-top:8px;">
            <div class="param"><label>Cant. (${a.unidad})</label><input type="number" min="0" step="0.5" value="${a.cantidad}" onchange="updAdicional('${a.id_temp}','cantidad',this.value)"></div>
